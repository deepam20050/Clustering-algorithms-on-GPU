...
int main() {
  const string filename = "/home/deepam/Desktop/SEM 6/CSE560 - GPU Computing/Clustering-algorithms-on-GPU/CPU/datasets/dataset5000.csv";
  auto data = csv_parser(filename);
  const int N = static_cast<int>(data.size());
  auto model = cluster_cpu(data, N);
  model.build_r_tree();
  auto v = model.DBSCAN(0.5, 4);
